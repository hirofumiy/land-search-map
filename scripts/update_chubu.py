#!/usr/bin/env python3
"""
substations.json 中部電力パワーグリッドエントリ更新スクリプト
====================================================
中部電力PG公式CSV（WebGIS gridmap配信・変電所空容量一覧）から
空容量(当該設備)を取得し、中部エントリ858件を更新する。
...
使い方:
  python3 scripts/update_chubu.py <csv_dir>
"""
...
def load_csvs(csv_dir: Path):
    ...
    for path in sorted(csv_dir.glob("*.csv")):
        ...
        with open(path, encoding=enc, errors='replace') as fh:
            ...
            rows = list(reader)
        if rows and ('作成' in rows[0][0] or '更新' in rows[0][0]):
        ...
        if avail_i is None:
            avail_i = next((i for n, i in col.items() if '空容量' in n), None)
        count = 0
...
def main():
    ...
    for f in feats:
        ...
        p["notes"] = (f"【公式データ 中部電力PG {data_date or '2026年3〜5月'}】予想潮流等情報CSVより。"
    ...
    meta.setdefault("company_data_updates", {})[COMPANY] = {
        ...
        "source": "変電所空容量・予想潮流一覧表CSV（本店+6エリア）",
    ...
    print("\n--- 未マッチサンプル ---")
    print(" ", unmatched[:20])
    log_path = Path(__file__).parent / f"update_log_chubu_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
