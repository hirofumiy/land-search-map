#!/usr/bin/env python3
"""
substations.json 北海道電力NWエントリ更新スクリプト
====================================================
北海道電力NW公式CSV（混雑状況マップ・予想潮流等一覧表 変圧器）から
空容量(当該設備)を取得し、北海道エントリ294件を更新する。
...
使い方:
  python3 scripts/update_hokkaido.py <csv_dir>
"""
...
def main():
    ...
    for f in feats:
        ...
        p["notes"] = (f"【公式データ 北海道電力NW {data_date or '2026年3〜5月'}】予想潮流等情報CSVより。"
    ...
    meta.setdefault("company_data_updates", {})[COMPANY] = {
        ...
        "source": "混雑状況マップ・予想潮流等一覧表 変圧器CSV（local01〜07）",
    ...
    print("\n--- 未マッチサンプル ---")
    print(" ", unmatched[:20])
    log_path = Path(__file__).parent / f"update_log_hokkaido_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
