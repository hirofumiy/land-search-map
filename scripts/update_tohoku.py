#!/usr/bin/env python3
"""
substations.json 東北電力NWエントリ更新スクリプト
====================================================
東北電力NW公式CSV（予想潮流等に関する情報・変圧器）から
空容量(当該設備)を取得し、東北エントリ582件を再処理する。
D率99.3%異常データの是正が目的。
...
使い方:
  python3 scripts/update_tohoku.py <csv_dir>
"""
...
def main():
    ...
    for f in feats:
        ...
        p["notes"] = (f"【公式データ 東北電力NW {data_date or '2026年3〜5月'}】予想潮流等情報CSVより。"
    ...
    meta.setdefault("company_data_updates", {})[COMPANY] = {
        ...
        "source": "予想潮流等に関する情報 変圧器CSV（local01〜07）",
    ...
    print("\n--- 未マッチサンプル ---")
    print(" ", unmatched[:20])
    log_path = Path(__file__).parent / f"update_log_tohoku_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
