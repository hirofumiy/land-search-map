#!/usr/bin/env python3
"""
substations.json 北陸電力送配電エントリ更新スクリプト
====================================================
北陸電力送配電の公式CSV（予想潮流等一覧表 変圧器＝変電所別）から
空き容量[MW]を取得し、北陸エントリ121件を更新する。
※北陸のCSVは列名が「空き容量[MW]」（"き"付き・上位系考慮列は無い）。

ランク変換閾値（2026-06-10 ひろ社長承認）:
  S: 20MW以上 / A: 10〜19 / B: 5〜9 / C: 1〜4 / D: 0以下

使い方:
  python3 scripts/update_hokuriku.py <csv_dir>
"""
...
def load_csvs(csv_dir: Path):
    """変圧器CSVから (短縮名) → row。重複時は空き容量が大きい方を採用"""
    lookup = {}
    data_date = None
    for path in sorted(csv_dir.glob("*.csv")):
        with open(path, encoding='cp932', errors='replace') as fh:
            rows = list(csv.reader(fh))
        # 1行目の日付表記（更新/作成/時点）を拾う
        if rows and rows[0] and any(k in rows[0][0] for k in ('更新', '作成', '時点')):
        ...
        name_i = col.get('変電所名')
        if name_i is None:
            name_i = next((i for n, i in col.items() if '変電所名' in n), None)
        v1_i = next((i for n, i in col.items() if '一次' in n), None)
        v2_i = next((i for n, i in col.items() if '二次' in n), None)
        units_i = col.get('台数')
        cap_i = next((i for n, i in col.items() if '設備容量' in n), None)
        op_i  = next((i for n, i in col.items() if '運用容量' in n), None)
        # 空き容量列：「空」「容量」を含み、設備/運用/上位系ではない列
        avail_i = next((i for n, i in col.items()
                        if '空' in n and '容量' in n
                        and '設備' not in n and '運用' not in n and '上位' not in n), None)
        if name_i is None or avail_i is None:
            continue
        count = 0
...
def main():
    ...
    print("⚡ 北陸電力送配電 変圧器CSV読み込み")
    lookup, data_date = load_csvs(csv_dir)
    print(f"  変電所ユニーク数: {len(lookup)} / データ日付: {data_date}")
    ...
    for f in feats:
        ...
        if new_mw is None:
            no_data += 1
            continue  # 空き容量「-」（熱容量制約等）は旧値維持せずゼロ扱いにしないため据置
        ...
        p["notes"] = (f"【公式データ 北陸電力送配電 {data_date or '2026年8月5日更新'}】予想潮流等一覧表（変圧器）より。"
                      f"{detail} 空き容量{new_mw}MW")
        updated += 1
    ...
    print(f"更新: {updated}件 / 空き非公開(-,旧値維持): {no_data}件 / 未マッチ: {len(unmatched)}件 / ランク変更: {len(rank_changes)}件")
    ...
    print("\n--- 未マッチサンプル ---")
    print(" ", unmatched[:20])
    log_path = Path(__file__).parent / f"update_log_hokuriku_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
