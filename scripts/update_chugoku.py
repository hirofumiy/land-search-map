#!/usr/bin/env python3
"""
substations.json 中国電力ネットワークエントリ更新スクリプト
====================================================
中国電力NW公式CSV（予想潮流・空容量 変圧器 全県）から
空容量(当該設備)を取得し、中国エントリ353件を更新する。
...
使い方:
  python3 scripts/update_chugoku.py <csv_dir>
"""
...
def main():
    ...
    for f in feats:
        ...
        p["notes"] = (f"【公式データ 中国電力NW {data_date or '2026年3〜5月'}】予想潮流等情報CSVより。"
    ...
    print("\n--- 未マッチサンプル ---")
    print(" ", unmatched[:20])
    log_path = Path(__file__).parent / f"update_log_chugoku_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
