#!/usr/bin/env python3
"""
substations.json 関西電力送配電エントリ更新スクリプト
====================================================
関西電力送配電公式CSV（変圧器空容量一覧 154kV未満・以上）から
空容量(当該設備)を取得し、関西エントリ559件を更新する。
...
使い方:
  python3 scripts/update_kansai.py <csv_dir>
"""
...
def main():
    ...
    for f in feats:
        ...
        p["notes"] = (f"【公式データ 関西電力送配電 {data_date or '2026年3〜5月'}】予想潮流等情報CSVより。"
    ...
    print("\n--- 未マッチサンプル ---")
    print(" ", unmatched[:20])
    log_path = Path(__file__).parent / f"update_log_kansai_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
